...
class dqn:
    ...
    def learn(self):
		...
		# estimate q values for this and next states
        q_eval = self.sess.run(self.q_eval, feed_dict={self.s: batch_memory[:, :self.n_features]})
        q_eval_next = self.sess.run(self.q_eval, feed_dict={self.s: batch_memory[:, -self.n_features:]})
        ...
        q_target[batch_index, eval_act_index] = reward + self.gamma * np.max(q_eval_next, axis=1)
