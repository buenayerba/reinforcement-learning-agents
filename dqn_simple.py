...
# Note: referred to an example from https://github.com/MorvanZhou/ 
...
class dqn:
    ...
    def learn(self, prev_obs, action, reward, new_obs):
        q_eval = self.getQ(prev_obs)
        q_eval_next = self.getQ(new_obs)        
        q_target = q_eval
        q_target[0, action] = reward + self.gamma * np.max(q_eval_next)
